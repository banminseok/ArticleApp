    ...
    public class UploadRepositoryAsyncTest
    {
        ...
        public async Task UploadRepositoryAsyncAllMethodTest()
        {
            ...
            using (var context = new ArticleAppDbContext(options))
            {
                ...
                //[A] Arrange
                // '{'CreatedBy', 'FileName', 'ModifiedBy'}' are missing for the instance of entity type 'Upload'. Consider using 'DbContextOptionsBuilder.EnableSensitiveDataLogging' to see the entity key value.
            ...
            using (var context = new ArticleAppDbContext(options))
            {
                ...
                //[A] Arrange
                var repository = new UploadRepository(context, factory);
                var model = new Upload { Name = "[2] 홍길동", Title = "공지사항입니다.", Content = "내용입니다.", ParentId = 3, ParentKey = "1" };

                //[B] Act
                await repository.AddAsync(model); // Id: 2
                await context.SaveChangesAsync(); // 메모리 DB 사용 시 필요
                await repository.AddAsync(new Upload { Name = "[3] 백두산", Title = "자유게시판입니다.", Content = "내용입니다.", ParentId = 3, ParentKey = "1" }); // Id: 3
                await context.SaveChangesAsync(); // 메모리 DB 사용 시 필요
            }
            using (var context = new ArticleAppDbContext(options))
            {
